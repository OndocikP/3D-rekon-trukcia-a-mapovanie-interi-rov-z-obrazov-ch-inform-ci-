...
def run_yolo_detection(project_id: str, user_id: str):
    """Background task na spustenie YOLO detekcie objektov"""
    try:
        print(f"🤖 YOLO Background Task: Spúšťam detekciu pre projekt {project_id}")
        utils = BackendUtils()
        detected_objects = utils.detect_objects_in_project(project_id, user_id)
        if detected_objects:
            print(f"✅ YOLO Background Task: Detekcia ukončená - {detected_objects}")
        else:
            print(f"❌ YOLO Background Task: Žiadne objekty nenájdené")
        utils.close()
    except Exception as e:
        print(f"❌ YOLO Background Task CHYBA: {str(e)}")
...
async def create_project(
    ...
    """Vytvorenie nového projektu - automaticky spúšťa YOLO detekciu"""
    ...
    # 🤖 Spusti YOLO detekciu na pozadí
    background_tasks.add_task(run_yolo_detection, str(db_project.id), str(current_user.id))
    print(f"📩 YOLO background task zaradený pre projekt: {db_project.id}")
