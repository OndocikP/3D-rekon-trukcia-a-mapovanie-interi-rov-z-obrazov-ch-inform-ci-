...
export default function ProjectNewScreen() {
  ...
  const goGenerate = async () => {
    ...
    setLoading(true);
    try {
      // 1. Vytvor projekt - volaj nový backend endpoint
      console.log('📁 Vytváram projekt:', projectName);
      console.log('🔗 API_URL:', API_URL);
      console.log('👤 User ID:', user?.id);
      console.log('📨 Request body:', { name: projectName, description: description || null });
      
      const createResponse = await fetch(`${API_URL}/projects/create/${user.id}`, {
        method: 'POST',
        headers: {
      ...
      if (!createResponse.ok) {
        ...
        throw new Error(error.detail || 'Chyba pri vytváraní projektu');
      }

      const newProject = await createResponse.json();
      console.log('✅ Projekt vytvorený:', newProject);

      const projectId = newProject.id;

      // 2. Nahraj obrázky (voliteľne)
      if (images.length > 0) {
        setUploading(true);
        let successCount = 0;
        let failedCount = 0;
        ...
        if (failedCount > 0) {
          Alert.alert(
            'Čiastočný úspech',
            `Projekt vytvorený!\n\n✅ ${successCount} obrázkov OK\n❌ ${failedCount} zlyhalo`
          );
        }
      } else {
        console.log('ℹ️ Žiadne obrázky na nahrávanie');
      }

      Alert.alert('Úspech', `Projekt "${projectName}" bol vytvorený!`);
      
      // Presmeruj na project detail
